...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['course_program_session'])) {
        ...
        list($selectedCourse, $selectedProgram,$selectedSession,$selectedRole) = explode(' ', $_POST['course_program_session']);

        // Redirect to teacher_course_list.php with the selected teacher's first name and last name as parameters
        header("Location: mark_attendance.php?course=$selectedCourse&program=$selectedProgram&session=$selectedSession&role=$selectedRole");
        exit();
    } else {
        echo "<script>alert('Please select/add a course.');</script>";
    }
}
?>
<!DOCTYPE html>
        ...
        <h2>Selected Teacher's Course Information:</h2>
            <form method="post">
                <?php
                    ...
                    echo '<b><i>Courses of '. $selectedFirstName . ' ' . $selectedLastName . ' (Role: ' . $selectedRole . ')</b></i>';
                    ...
                    if (!$result) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<td><input type="radio" name="course_program_session" value="' . $row['subject'] . ' ' . $row['program'] . ' ' .$row['session'] . ' ' . $selectedRole . '"></td>';
                            echo '</tr>';
                        }
                        echo '</table>';
                    } else {
