...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['course_program_session'])) {
        ...
        list($selectedCourse, $selectedProgram,$selectedSession,$role,$email) = explode(' ', $_POST['course_program_session']);

        // Redirect to teacher_course_list.php with the selected teacher's first name and last name as parameters
        header("Location: view_attendance.php?course=$selectedCourse&program=$selectedProgram&session=$selectedSession&role=$role&email=$email");
        exit();
    } else {
...
<body>
    <div class="container">
        <h2>Student's Course Information:</h2>
            <form method="post">
                <?php
                    ...
                    echo '<b><i> Please select a Course : </b></i>';
                    $result = $conn->query($query);

                    if ($result->num_rows > 0) {
                        echo '<table>';
                        echo '<tr><th>Course</th><th>Select</th></tr>';
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr>';
                            echo '<td>' . $row['subject'] . '</td>';
                            echo '<td><input type="radio" name="course_program_session" value="' . $row['subject'] . ' ' . $row['program'] . ' ' .$row['session'] . ' ' . $role . ' ' . $email . '"></td>';
                            echo '</tr>';
                        }
                        echo '</table>';
                    } else {
                        echo "No courses found for the selected student.";
                    }
